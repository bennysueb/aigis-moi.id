...
<div class="container mx-auto px-4 py-12">
    <div class="text-center mb-10">
        <h1 class="text-3xl font-bold text-gray-900">Agenda Acara</h1>
        <p class="text-gray-600">Jadwal kegiatan dan sesi acara mendatang.</p>
    </div>

    @if($groupedAgendas->isEmpty())
        <div class="text-center py-10 bg-gray-50 rounded-lg">
            <p class="text-gray-500">Belum ada agenda acara yang dijadwalkan saat ini.</p>
        </div>
    @else
        <div class="max-w-4xl mx-auto">
            @foreach($groupedAgendas as $date => $agendas)
                <div class="mb-12">
                    <div class="sticky top-0 bg-white z-10 py-2 border-b-2 border-blue-500 mb-6">
                    ...
                    <div class="space-y-6">
                        @foreach($agendas as $agenda)
                            <div class="flex flex-col md:flex-row bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-gray-100">
                                <div class="bg-blue-50 p-4 md:w-1/4 flex flex-col justify-center items-center text-center border-r border-blue-100">
                                    <span class="text-lg font-bold text-blue-800">{{ $agenda->start_time->format('H:i') }}</span>
                                    <span class="text-sm text-gray-500">s/d</span>
                                    <span class="text-lg font-bold text-blue-800">{{ $agenda->end_time->format('H:i') }}</span>
                                </div>

                                <div class="p-5 md:w-3/4 flex flex-col justify-between">
                                    <div>
                                        <div class="flex justify-between items-start">
                                            <h4 class="text-lg font-bold text-gray-800 mb-1">{{ $agenda->title }}</h4>
                                    ...
                                    {{-- TOMBOL LINK (Sesuai Request) --}}
                                    @if($agenda->link_url)
                                        <div class="mt-4 pt-3 border-t border-gray-100 text-right">
                                            <a href="{{ $agenda->link_url }}" target="_blank" class="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors">
                                                Lihat Detail / Tautan
