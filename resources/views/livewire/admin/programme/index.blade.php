<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <div class="flex justify-between mb-4">
                <h2 class="text-xl font-bold text-gray-800">Kelola Event Programme</h2>
            ...
            <table class="table-auto w-full border-collapse border border-gray-200">
                <thead>
                    <tr class="bg-gray-100">
                        <th class="px-4 py-2 border">Events</th>
                    ...
                    <tr>
                        <td class="border px-4 py-2">
                            ...
                            <div class="flex flex-wrap gap-1">
                                ...
                                <span class="bg-purple-100 text-purple-800 py-1 px-2 rounded text-xs">
                        ...
                        <td class="border px-4 py-2">
                            {{ $prog->start_time->format('d M Y') }}<br>
                            <span class="text-sm text-gray-500">
                                {{ $prog->start_time->format('H:i') }} - {{ $prog->end_time->format('H:i') }}
                            </span>
                        </td>
                        <td class="border px-4 py-2">
                            <div class="font-bold">{{ $prog->title }}</div>
                            <div class="text-xs text-gray-500">{{ Str::limit($prog->description, 50) }}</div>
                        </td>
                        <td class="border px-4 py-2">
                            <div>📍 {{ $prog->location ?? '-' }}</div>
                            <div>🎤 {{ $prog->speaker ?? '-' }}</div>
                        </td>
                        <td class="border px-4 py-2 text-center">
                            @if($prog->link_url)
                            <a href="{{ $prog->link_url }}" target="_blank" class="text-blue-500 underline">Link</a>
                            @else
                            -
                            @endif
                        </td>
                        <td class="border px-4 py-2 text-center">
                            <button wire:click="edit({{ $prog->id }})" class="bg-yellow-500 text-white px-2 py-1 rounded text-sm">Edit</button>
                            <button wire:click="delete({{ $prog->id }})" onclick="return confirm('Yakin hapus?')" class="bg-red-500 text-white px-2 py-1 rounded text-sm">Hapus</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <div class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
        <div class="bg-white rounded shadow-lg w-1/2 p-6 overflow-y-auto max-h-[90vh]">
            <h2 class="text-xl mb-4 font-bold">{{ $programmeId ? 'Edit Program' : 'Tambah Program' }}</h2>
            ...
            <form>
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Tahan tombol <strong>Ctrl</strong> (Windows) atau <strong>Command</strong> (Mac) untuk memilih lebih dari satu. Biarkan kosong jika program umum.</p>
                    @error('event_ids') <span class="text-red-500 text-sm">{{ $message }}</span>@enderror
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-bold mb-2">Judul Program</label>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-bold mb-2">Pengisi Acara</label>
